...
# 配置 - 遵循 yuppy-skills 项目规则
SKILL_DIR = os.path.dirname(os.path.abspath(__file__))
YUPPY_SKILLS_DIR = '/Users/roc/.yuppy-skills'
MEDIA_DOWNLOADER_DIR = os.path.join(YUPPY_SKILLS_DIR, 'media-downloader')
DOWNLOADS_DIR = os.path.join(MEDIA_DOWNLOADER_DIR, 'downloads')
CACHE_DIR = os.path.join(MEDIA_DOWNLOADER_DIR, 'cache')
CONFIG_DIR = os.path.join(MEDIA_DOWNLOADER_DIR, 'config')

# 确保目录存在
os.makedirs(MEDIA_DOWNLOADER_DIR, exist_ok=True)
os.makedirs(DOWNLOADS_DIR, exist_ok=True)
os.makedirs(CACHE_DIR, exist_ok=True)
os.makedirs(CONFIG_DIR, exist_ok=True)

# API Keys (可以从环境变量获取，或从配置文件读取)
PEXELS_API_KEY = os.environ.get('PEXELS_API_KEY', '')
UNSPLASH_ACCESS_KEY = os.environ.get('UNSPLASH_ACCESS_KEY', '')
PIXABAY_API_KEY = os.environ.get('PIXABAY_API_KEY', '')

# 检查是否有本地配置文件
api_config_file = os.path.join(CONFIG_DIR, 'api_keys.json')
if os.path.exists(api_config_file):
    try:
        with open(api_config_file, 'r', encoding='utf-8') as f:
            api_config = json.load(f)
        PEXELS_API_KEY = api_config.get('PEXELS_API_KEY', PEXELS_API_KEY)
        UNSPLASH_ACCESS_KEY = api_config.get('UNSPLASH_ACCESS_KEY', UNSPLASH_ACCESS_KEY)
        PIXABAY_API_KEY = api_config.get('PIXABAY_API_KEY', PIXABAY_API_KEY)
...
def cmd_config(args):
    ...
    if args.set:
        key, value = args.set
        key = key.upper()
        if key in ["PEXELS_API_KEY", "UNSPLASH_ACCESS_KEY", "PIXABAY_API_KEY"]:
    ...
    if args.list:
        print("📋 当前 API 密钥配置:")
        for key in ["PEXELS_API_KEY", "UNSPLASH_ACCESS_KEY", "PIXABAY_API_KEY"]:
            value = api_config.get(key, "未设置")
            if len(value) > 10:
                value = value[:8] + "..."
            print(f"   {key}: {value}")
        return 0
    ...
    print("使用方法:")
    print("  python media_cli.py config --set PEXELS_API_KEY your_key")
    print("  python media_cli.py config --list")
    return 0
...
def main():
    ...
    # config command / 配置命令
    config_parser = subparsers.add_parser("config", help="Configure API keys / 配置 API 密钥")
    config_parser.add_argument("--set", nargs=2, metavar=("KEY", "VALUE"), help="Set API key (e.g., --set PEXELS_API_KEY your_key)")
    config_parser.add_argument("--list", action="store_true", help="List current API keys / 列出当前 API 密钥")
